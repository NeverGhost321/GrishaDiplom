...
- **unit/service** тесты на Vitest (`npm test`);
- **e2e** тесты на Playwright (`npm run test:e2e`).
...
Дополнительно для локальной отладки e2e:

```bash
npm run test:e2e:ui
```

E2E покрывают ключевые пользовательские сценарии:
- главная страница и переходы по CTA (auto-build/manual-build/saved-builds/components);
- каталог комплектующих (загрузка, категории, поиск/фильтры);
- автоматическая сборка (генерация, проверка результата, сохранение);
- ручная сборка (выбор компонентов, проверка совместимости, сохранение);
- сохранённые сборки (открытие деталей, кнопки JSON/PDF, удаление с подтверждением).

> Для стабильности e2e добавлены точечные `data-testid` на CTA главной страницы.
...
## Страница ручной сборки

Добавлена полноценная страница ` /manual-build ` для ручной конфигурации игрового ПК.

Пользователь вручную выбирает все ключевые категории:
- процессор;
- материнскую плату;
- оперативную память;
- видеокарту;
...
После выбора всех компонентов интерфейс автоматически отправляет запрос в `POST /api/compatibility/check` и показывает:
- итоговую стоимость сборки;
- суммарное энергопотребление;
- рекомендуемую мощность БП;
- compatibility score;
- критические ошибки, предупреждения и рекомендации.

Дополнительно на странице доступны:
- **сохранение сборки** через `POST /api/builds` (только для конфигураций без критических ошибок);
- **экспорт JSON** текущей ручной конфигурации и результата совместимости в файл `pc-manual-build.json`.

## Каталог комплектующих

Добавлена страница `GET /catalog` для демонстрационного просмотра всех seed-комплектующих проекта.

### Категории

На странице можно переключаться между категориями:
- Процессоры;
- Материнские платы;
- Оперативная память;
- Видеокарты;
...
Для каждой категории используется соответствующий API endpoint `/api/components/*`.

### Поиск, фильтры и сортировка

Поддерживаются параметры:
- `search` — поиск по модели/производителю;
- `manufacturer` — фильтр по бренду;
- `minPrice`, `maxPrice` — диапазон цены;
- `sort`:
  - `price_asc` — сначала дешёвые;
  - `price_desc` — сначала дорогие;
  - `manufacturer_asc` — сортировка по производителю.

На странице реализованы состояния загрузки, ошибки и пустого результата, выводится количество найденных позиций.

### Источник данных

Каталог работает на seed-данных (`prisma/seed.ts`) и предназначен для демонстрации алгоритмов совместимости и автоподбора в рамках ВКР.
