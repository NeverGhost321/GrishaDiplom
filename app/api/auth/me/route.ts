...
export async function GET() {
  ...
  return NextResponse.json({ user: { id: user.id, email: user.email, name: user.name, bio: user.bio, avatarUrl: user.avatarUrl, buildsCount } });
}
...
export async function PATCH(request: Request) {
  ...
  return NextResponse.json({ user: { id: updated.id, email: updated.email, name: updated.name, bio: updated.bio, avatarUrl: updated.avatarUrl } });
}
